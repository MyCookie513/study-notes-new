...
public class ConPool implements PoolService,Runnable{
    ...
    private int free;
    private int active;
    private static int number=10;//总共的线程数量
    private volatile boolean b=true;
    private ConPool(int init) {
    	number=init;
    	Init();
    }
    public static ConPool getPool(int num) {
        return  InstanceHolder.instance;
    }
    private static  class InstanceHolder{

        private static final ConPool instance =new ConPool(number);
    ...
    public Connection_DIY getConnection() {
    	  for(Iterator<Connection_DIY> it = list.iterator(); it.hasNext();){
    		  Connection_DIY connection_diy=it.next();
              if(connection_diy.connectionState== ConnectionState.FREE){
                 return connection_diy;
              }
      }
    	  number++;
    	  active++;
    ...
    @Override
    public void closeAllConnetions() throws SQLException {
    	b=false;
        for(Iterator<Connection_DIY> it = list.iterator(); it.hasNext();){
            Connection_DIY connection_diy=it.next();
            if(connection_diy.getConnection()!=null)
            	connection_diy.getConnection().close();
            it.remove();
